...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Input: Chinese text file, output: TeX file with Shanghainese ruby.')
    parser.add_argument('files', metavar='<text files>', type=str,
                        nargs='+', help='Please input a Traditional Chinese text file.')
        ...
        while counter < len(txt):
            if txt[counter] in d:
                ...
                while not txt[counter:counter+tr] in d:
                    tr -= 1
                out.append([txt[counter:counter+tr],
                            d[txt[counter:counter+tr]]])
                counter += tr
            else:
...
\end{document}''')
        fo.close()
        system(
            f"xelatex {file}.ruby.tex && rm {file}.ruby.aux && rm {file}.ruby.log")
